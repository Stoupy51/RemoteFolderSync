...
#define DEBUG_PRINT(...) { if (DEVELOPMENT_MODE) { if (errno != 0) { char* error_msg = strerror(errno); char buffer[16384]; sprintf(buffer, CYAN "[DEBUG] " RESET __VA_ARGS__); int err_pos = strlen(buffer); while (err_pos > 0 && buffer[err_pos] != '\n') err_pos--; buffer[err_pos] = '\0'; fprintf(stderr, "%s: %s\n", buffer, error_msg); errno = 0; } else { fprintf(stderr, CYAN "[DEBUG] " RESET __VA_ARGS__); } } }
