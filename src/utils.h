...
#define INFO_PRINT(...) { if (IS_INFO_LEVEL) { if (DEVELOPMENT_MODE) { if (errno != 0) { char buffer[16384]; sprintf(buffer, GREEN "[INFO] " RESET __VA_ARGS__); int err_pos = strlen(buffer); while (err_pos > 0 && buffer[err_pos] != '\n') err_pos--; buffer[err_pos] = '\0'; fprintf(stderr, "%s: %s\n", buffer, strerror(errno)); errno = 0; } else { fprintf(stderr, GREEN "[INFO] " RESET __VA_ARGS__); } } else { printf(GREEN "[INFO] " RESET __VA_ARGS__); } } }
#define DEBUG_PRINT(...) { if (IS_INFO_LEVEL) { if (DEVELOPMENT_MODE) { if (errno != 0) { char buffer[16384]; sprintf(buffer, CYAN "[DEBUG] " RESET __VA_ARGS__); int err_pos = strlen(buffer); while (err_pos > 0 && buffer[err_pos] != '\n') err_pos--; buffer[err_pos] = '\0'; fprintf(stderr, "%s: %s\n", buffer, strerror(errno)); errno = 0; } else { fprintf(stderr, CYAN "[DEBUG] " RESET __VA_ARGS__); } } else { printf(CYAN "[DEBUG] " RESET __VA_ARGS__); } } }
#define WARNING_PRINT(...) { if (IS_WARNING_LEVEL) { if (errno != 0) { char buffer[16384]; sprintf(buffer, YELLOW "[WARNING] " RESET __VA_ARGS__); int err_pos = strlen(buffer); while (err_pos > 0 && buffer[err_pos] != '\n') err_pos--; buffer[err_pos] = '\0'; fprintf(stderr, "%s: %s\n", buffer, strerror(errno)); errno = 0; } else { fprintf(stderr, YELLOW "[WARNING] " RESET __VA_ARGS__); } } }
#define ERROR_PRINT(...) { if (IS_ERROR_LEVEL) { if (errno != 0) { char buffer[16384]; sprintf(buffer, RED "[ERROR] " RESET __VA_ARGS__); int err_pos = strlen(buffer); while (err_pos > 0 && buffer[err_pos] != '\n') err_pos--; buffer[err_pos] = '\0'; fprintf(stderr, "%s: %s\n", buffer, strerror(errno)); errno = 0; } else { fprintf(stderr, RED "[ERROR] " RESET __VA_ARGS__); } } }
